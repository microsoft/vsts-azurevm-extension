...
def get_deployment_type(vsts_url, pat_token):
    ...
    if response.status == Constants.HTTP_OK:
        ...
        handler_utility.log(
            "Failed to fetch the connection data for the url {0}. Reason : {1} {2}".format(rest_call_url, str(response.status), response.reason)
        )
        return "hosted"
...
def validate_inputs(config):
    try:
        ...
        get_deployment_group_url = "{0}/{1}/_apis/distributedtask/deploymentgroups?name={2}&api-version={3}".format(
            config["VSTSUrl"], quote(config["TeamProject"]), quote(config["DeploymentGroup"]), Constants.projectAPIVersion
        )
        ...
        if ("value" not in deployment_group_data) or len(deployment_group_data["value"]) == 0:
            ...
            raise RMExtensionStatus.new_handler_terminating_error(
                inputs_validation_error_code, error_message_initial_part.format(response.status, specific_error_message)
            )
        ...
        patch_deployment_group_url = "{0}/{1}/_apis/distributedtask/deploymentgroups/{2}?api-version={3}".format(
            config["VSTSUrl"], quote(config["TeamProject"]), deployment_group_id, Constants.projectAPIVersion
        )
        ...
        if response.status != Constants.HTTP_OK:
            ...
            raise RMExtensionStatus.new_handler_terminating_error(
                inputs_validation_error_code, error_message_initial_part.format(response.status, response.reason) + specific_error_message
            )
...
def get_configuration_from_settings():
    try:
        ...
        set_error_status_and_error_exit(
            e, RMExtensionStatus.rm_extension_status["ReadingSettings"]["operationName"], RMExtensionStatus.rm_extension_status["InputConfigurationError"]
        )
...
def test_agent_configuration_required(config):
    ...
    agent_reconfiguration_required = ConfigureDeploymentAgent.is_agent_configuration_required(
        config["VSTSUrl"], config["PATToken"], config["DeploymentGroup"], config["TeamProject"], config["AgentWorkingFolder"]
    )
...
def register_agent(config):
    ...
    try:
        ...
        ConfigureDeploymentAgent.configure_agent(
            config["VSTSUrl"],
            config["PATToken"],
            config["TeamProject"],
            config["DeploymentGroup"],
            config["ConfigureAgentAsUserName"],
            config["AgentName"],
...
def add_agent_tags(config):
    ...
    try:
        ...
        if config["Tags"] != None and len(config["Tags"]) > 0:
            ...
            ConfigureDeploymentAgent.add_agent_tags(config["VSTSUrl"], config["TeamProject"], config["PATToken"], config["AgentWorkingFolder"], tags_string)
