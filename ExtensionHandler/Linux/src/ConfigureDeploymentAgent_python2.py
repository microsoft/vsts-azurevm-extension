...
def is_agent_configuration_required(vsts_url, pat_token, deployment_group_name, project_name, working_folder):
    ...
    try:
        ...
        if (
            existing_vsts_url.lower() == vsts_url_for_configuration.lower()
            and existing_deployment_group_data["name"].lower() == deployment_group_name.lower()
            and existing_deployment_group_data["project"]["name"].lower() == project_name.lower()
        ):
...
def add_agent_tags(vsts_url, project_name, pat_token, working_folder, tags_string):
    try:
        ...
        if agent_id == "" or project_id == "" or deployment_group_id == "":
            raise Exception(
                "Unable to get one or more of the project id, deployment group id, or the agent id. Ensure that the agent is configured before addding tags."
            )
...
def remove_existing_agent(working_folder):
    try:
        ...
        service_stop_proc = subprocess.Popen(
            "{0} stop".format(agent_service_path).split(" "), stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=working_folder
        )
        ...
        service_uninstall_proc = subprocess.Popen(
            "{0} uninstall".format(agent_service_path).split(" "), stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=working_folder
        )
...
def _get_deployment_group_data_from_setting(vsts_url, pat_token):
    ...
    if project_id != "" and deployment_group_id != "":
        deployment_group_data_address = "/{0}/_apis/distributedtask/deploymentgroups/{1}?api-version={2}".format(
            project_id, deployment_group_id, Constants.targets_api_version
        )
...
def _apply_tags_to_agent(vsts_url, pat_token, project_id, deployment_group_id, agent_id, tags_string):
    tags_address = "/{0}/_apis/distributedtask/deploymentgroups/{1}/Targets?api-version={2}".format(
        project_id, deployment_group_id, Constants.targets_api_version
    )
...
def _add_tags_to_agent_internal(vsts_url, pat_token, project_id, deployment_group_id, agent_id, tags_string):
    target_address = "/{0}/_apis/distributedtask/deploymentgroups/{1}/Targets/{2}?api-version={3}".format(
        project_id, deployment_group_id, agent_id, Constants.targets_api_version
    )
    ...
    if response.status == Constants.HTTP_OK:
        ...
        raise Exception(
            "Tags could not be added. Unable to fetch the existing tags or deployment group details: {0} {1}".format(str(response.status), response.reason)
        )
...
def _configure_agent_internal(vsts_url, pat_token, project_name, deployment_group_name, configure_agent_as_username, agent_name, working_folder):
    ...
    configure_command_args = [
        ...
        agent_name,
        "--work",
        Constants.default_agent_work_dir,
        "--projectname",
        project_name,
        "--deploymentgroupname",
    ...
    config_agent_proc = subprocess.Popen(
        "{0} configure --unattended --acceptteeeula --deploymentgroup --replace".format(agent_listener_path).split(" ") + configure_command_args,
