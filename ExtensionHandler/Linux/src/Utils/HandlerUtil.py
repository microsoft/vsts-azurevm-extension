...
class HandlerUtility:
    ...
    def set_handler_error_status(self, e, operation_name):
        ...
        if(error_code == RMExtensionStatus.rm_extension_status['InstallError']):
            error_message = 'The Extension failed to install: {0} More information about the failure can be found in the logs located under {1} on the VM.To retry install, please remove the extension from the VM first.'.format(e.message, self._context._log_dir)
        elif(error_code == RMExtensionStatus.rm_extension_status['ArgumentError']):
            error_message = 'The Extension received an incorrect input: {0} Please correct the input and retry executing the extension.'.format(e.message)
        else:
            error_message = 'The Extension failed to execute: {0} More information about the failure can be found in the logs located under {1} on the VM.'.format(e.message, self._context._log_dir)
        self.set_handler_status(code = error_code, message = error_message, status = 'error')
        self.set_handler_status(ss_code = error_code, sub_status_message = error_message, sub_status = 'error', operation_name = operation_name)
