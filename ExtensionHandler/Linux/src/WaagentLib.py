...
global RulesFiles
RulesFiles = ["/lib/udev/rules.d/75-persistent-net-generator.rules", "/etc/udev/rules.d/70-persistent-net.rules"]
...
class AbstractDistro(object):
    ...
    def __init__(self):
        ...
        self.requiredDeps = ["route", "shutdown", "ssh-keygen", "useradd", "usermod", "openssl", "sfdisk", "fdisk", "mkfs", "sed", "grep", "sudo", "parted"]
    ...
    def changePass(self, user, password):
        ...
        return self.chpasswd(user, password, crypt_id=crypt_id, salt_len=salt_len)
    ...
    def getInterfaceNameByMac(self, mac):
        ...
        match = re.search(r"(eth\d).*(HWaddr|ether) {0}".format(mac), output, re.IGNORECASE)
        if match is None:
    ...
    def configIpV4(self, ifName, addr, netmask=24):
        ...
        if ret != 0:
            raise Exception("Failed to bring up {0}: {1}".format(ifName, output))
        ret, output = RunGetOutput("ifconfig {0} {1}/{2}".format(ifName, addr, netmask))
        if ret != 0:
            raise Exception("Failed to config ipv4 for {0}: {1}".format(ifName, output))
...
class FreeBSDDistro(AbstractDistro):
    ...
    def __init__(self):
        ...
        self.hostname_file_path = "/etc/hostname"
        self.dhcp_client_name = "dhclient"
        self.requiredDeps = ["route", "shutdown", "ssh-keygen", "pw", "openssl", "fdisk", "sed", "grep", "sudo"]
        ...
        try:
            ...
            if not os.path.isdir(MyDistro.sudoers_dir_base + "/sudoers.d/"):
                ...
                SetFileContents(
                    ...
                    GetFileContents(MyDistro.sudoers_dir_base + "/sudoers") + "\n#includedir " + MyDistro.sudoers_dir_base + "/sudoers.d\n",
                )
            if password == None:
...
def LoggerInit(log_file_path, log_con_path, verbose=False):
    ...
    Log, LogWithPrefix, LogIfVerbose, LogWithPrefixIfVerbose, Error, ErrorWithPrefix, Warn, NoLog, ThrottleLog, myLogger = (
        l.Log,
        l.LogWithPrefix,
        l.LogIfVerbose,
        l.LogWithPrefixIfVerbose,
        l.Error,
        l.ErrorWithPrefix,
        l.Warn,
        l.NoLog,
...
class Util(object):
    ...
    def _HttpRequest(self, method, host, path, port=None, data=None, secure=False, headers=None, proxyHost=None, proxyPort=None):
        resp = None
        conn = None
        try:
            if secure:
    ...
    def HttpRequest(self, method, url, data=None, headers=None, maxRetry=3, chkProxy=False):
        """
        ...
        if secure and proxyHost is not None and proxyPort is not None and not hasattr(httplibs.HTTPSConnection, "set_tunnel"):
            Warn("httplib doesn't support https tunnelling(new in python 2.7)")
            secure = False
            proxyHost, proxyPort = self.GetHttpProxy(secure)

        resp = self._HttpRequest(method, host, path, port=port, data=data, secure=secure, headers=headers, proxyHost=proxyHost, proxyPort=proxyPort)
        for retry in range(0, maxRetry):
            if resp is not None and (resp.status == httplibs.OK or resp.status == httplibs.CREATED or resp.status == httplibs.ACCEPTED):
                return resp
    ...
    def HttpGet(self, url, headers=None, maxRetry=3, chkProxy=False):
        return self.HttpRequest("GET", url, headers=headers, maxRetry=maxRetry, chkProxy=chkProxy)

    def HttpHead(self, url, headers=None, maxRetry=3, chkProxy=False):
        return self.HttpRequest("HEAD", url, headers=headers, maxRetry=maxRetry, chkProxy=chkProxy)

    def HttpPost(self, url, data, headers=None, maxRetry=3, chkProxy=False):
        return self.HttpRequest("POST", url, data=data, headers=headers, maxRetry=maxRetry, chkProxy=chkProxy)

    def HttpPut(self, url, data, headers=None, maxRetry=3, chkProxy=False):
        return self.HttpRequest("PUT", url, data=data, headers=headers, maxRetry=maxRetry, chkProxy=chkProxy)

    def HttpDelete(self, url, headers=None, maxRetry=3, chkProxy=False):
        return self.HttpRequest("DELETE", url, headers=headers, maxRetry=maxRetry, chkProxy=chkProxy)

    def HttpGetWithoutHeaders(self, url, maxRetry=3, chkProxy=False):
        """
        Return data from an HTTP get on 'url'.
        """
        resp = self.HttpGet(url, headers=None, maxRetry=maxRetry, chkProxy=chkProxy)
    ...
    def HttpGetWithHeaders(self, url, maxRetry=3, chkProxy=False):
        ...
        headers.
        """
        resp = self.HttpGet(url, headers={"x-ms-agent-name": GuestAgentName, "x-ms-version": ProtocolVersion}, maxRetry=maxRetry, chkProxy=chkProxy)
        return resp.read() if resp is not None else None
    ...
    def HttpPostWithHeaders(self, url, data, maxRetry=3, chkProxy=False):
        headers = {"x-ms-agent-name": GuestAgentName, "Content-Type": "text/xml; charset=utf-8", "x-ms-version": ProtocolVersion}
        try:
            return self.HttpPost(url, data=data, headers=headers, maxRetry=maxRetry, chkProxy=chkProxy)
...
def GetBlobType(url):
    ...
    blobPropResp = restutil.HttpHead(url, {"x-ms-date": timestamp, "x-ms-version": __StorageVersion}, chkProxy=True)
    blobType = None
...
def PutBlockBlob(url, data):
    ...
    timestamp = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    ret = restutil.HttpPut(
        url, data, {"x-ms-date": timestamp, "x-ms-blob-type": "BlockBlob", "Content-Length": str(len(data)), "x-ms-version": __StorageVersion}, chkProxy=True
    )
    if ret is None:
...
class Certificates(object):
    ...
    def Parse(self, xmlText):
        ...
        if Run(
            Openssl
            + " cms -decrypt -in Certificates.p7m -inkey TransportPrivate.pem -recip TransportCert.pem | "
            + Openssl
            + " pkcs12 -nodes -password pass: -out Certificates.pem"
        ):
...
class ExtensionsConfig(object):
        ...
        for p in self.Plugins:
            ...
            if plg_dir == None or LooseVersion(version) > LooseVersion(previous_version):
                ...
                if len(mp.getElementsByTagName("DisallowMajorVersionUpgrade")):
                    if (
                        GetNodeTextData(mp.getElementsByTagName("DisallowMajorVersionUpgrade")[0]) == "true"
                        and previous_version != None
                        and previous_version.split(".")[0] != version.split(".")[0]
                    ):
                ...
                handler_env = (
                    ...
                    + os.path.dirname(p.plugin_log)
                    + '",    "configFolder": "'
                    + root
                    + '/config",    "statusFolder": "'
                    + root
                    + '/status",    "heartbeatFile": "'
                    + root
                    + '/heartbeat.log"}}]'
                )
    ...
    def ReportHandlerStatus(self):
        ...
        status = (
            '{"version":"1.0","timestampUTC":"'
            + tstamp
            + '","aggregateStatus":{"guestAgentStatus":{"version":"'
            + GuestAgentVersion
            + '","status":"'
            + agent_state
    ...
    def GenerateAggStatus(self, name, version, reportHeartbeat=False):
        ...
        agg_status_obj = {"handlerName": name, "handlerVersion": version, "status": agg_state, "runtimeSettingsStatus": {"sequenceNumber": current_seq_no}}
        if status_obj:
...
class WALAEvent(object):
    ...
    def ToXml(self):
        strEventid = '<Event id="{0}"/>'.format(self.eventId)
        strProviderid = '<Provider id="{0}"/>'.format(self.providerId)
        strRecordFormat = '<Param Name="{0}" Value="{1}" T="{2}" />'
        strRecordNoQuoteFormat = '<Param Name="{0}" Value={1} T="{2}" />'
        strMtStr = "mt:wstr"
        strMtUInt64 = "mt:uint64"
        strMtBool = "mt:bool"
        strMtFloat = "mt:float64"
        strEventsData = ""
        ...
        for attName in self.__dict__:
            ...
            Log(
                ...
                + ":type"
                + str(type(attValue))
                + "Can't convert to events data:"
                + ":type not supported"
            )

        return "<Data>{0}{1}{2}</Data>".format(strProviderid, strEventid, strEventsData)
...
class WALAEventMonitor(WALAEvent):
    ...
    def EventsLoop(self):
        ...
        try:
            while True:
                if (datetime.datetime.now() - LastReportHeartBeatTime) > datetime.timedelta(minutes=30):
    ...
    def SendEvent(self, providerid, events):
        dataFormat = '<?xml version="1.0"?><TelemetryData version="1.0"><Provider id="{0}">{1}' "</Provider></TelemetryData>"
    ...
    def CollectAndSendWALAEvents(self):
        ...
        for file in eventFiles:
            ...
            eventstr = '<Event id="{0}"><![CDATA[{1}]]></Event>'.format(eventid, params)
