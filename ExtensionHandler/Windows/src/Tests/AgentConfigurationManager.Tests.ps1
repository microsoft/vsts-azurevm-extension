...
Describe "Agent Configuration Manager Tests" {
    ...
    Context "Agent config command" {
        ...
        It "should quote arguments containing spaces" {
            $expectedArgs = "--deploymentgroup --runasservice --unattended --auth PAT  --agent `"my agent`" --url `"https://acccount.visualstudio.com`" --token pat --work `"C:\work folder`" --projectname `"my proj`" --deploymentgroupname `"my dg`""
