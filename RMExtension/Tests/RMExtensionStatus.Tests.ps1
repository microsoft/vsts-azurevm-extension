...
Describe "Handler error status tests" {
    ...
    Context "Should set proper terminating error " {
        ...
        It "should have info about logs location" {
            $status.status.formattedMessage.message | Should BeLike 'The RM Extension failed to execute: The current CPU architecture is not supported. RM agent requires x64 architecture.*'
